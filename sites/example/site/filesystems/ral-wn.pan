...
variable DISK_VOLUME_PARAMS = filesystem_layout_mod(dict(
    'afscache', dict(
        ...
        'type', 'partition',
        'device', format('%s%s%s', DISK_BOOT_DEV, DISK_BOOT_PART_PREFIX, DISK_RAL_PART_OFFSET + index('afscache', DISK_RAL_PARTS) + 1),
    ),
    'homepool', dict(
        'size', DISK_RAL_AFSCACHE_SIZE,
        'mountpoint', DISK_RAL_AFSCACHE_MOUNTPOINT,
        'type', 'partition',
        'device', format('%s%s%s', DISK_BOOT_DEV, DISK_BOOT_PART_PREFIX, DISK_RAL_PART_OFFSET + index('homepool', DISK_RAL_PARTS) + 1),
    ),
    'pbspool', dict(
        'size', DISK_RAL_AFSCACHE_SIZE,
        'mountpoint', DISK_RAL_AFSCACHE_MOUNTPOINT,
        'type', 'partition',
        'device', format('%s%s%s', DISK_BOOT_DEV, DISK_BOOT_PART_PREFIX, DISK_RAL_PART_OFFSET + index('pbspool', DISK_RAL_PARTS) + 1),
    ),
    'pool', dict(
        'size', DISK_RAL_AFSCACHE_SIZE,
        'mountpoint', DISK_RAL_AFSCACHE_MOUNTPOINT,
        'type', 'partition',
        'device', format('%s%s%s', DISK_BOOT_DEV, DISK_BOOT_PART_PREFIX, DISK_RAL_PART_OFFSET + index('pool', DISK_RAL_PARTS) + 1),
    ),
));
